  ...
  return (
    <div className="eq-block" style={{ padding: "16px", overflow: "hidden" }}>
      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "10px", alignItems: "center" }}>
        ...
        <button style={{ ...btnStyle(playing), marginLeft: "12px" }} onClick={e => { e.stopPropagation(); setPlaying(p => !p); if (!playing) lastRef.current = 0; }}>
          {playing ? "Pause" : "Play"}
        </button>
      </div>
      <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", maxWidth: w, display: "block", margin: "0 auto" }}>
    ...
    content: (gp) => (
      <div className="lesson-body">
        ...
        <Section title="Loss Tangent">
          ...
          <P>For the Debye model, the loss tangent peaks at <M>{"\\omega = 1/\\tau"}</M> (i.e., <M>{"f_{\\text{peak}} = 1/(2\\pi\\tau)"}</M>).</P>
        </Section>
        ...
        <Section title="Cole-Cole Plot">
          ...
          <KeyConcept label="Deviations from Debye Behavior">
            Real materials often show depressed semicircles (center below the real axis), indicating a distribution of relaxation times rather than a single tau. This is described by the Cole-Cole parameter alpha (0 for ideal Debye, approaches 1 for very broad distribution).
          </KeyConcept>
        </Section>
      </div>
    ),
